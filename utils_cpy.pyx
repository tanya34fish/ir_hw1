...
class informationRetrieval():
    def __init__(self, docNum, queryList, vocab):
        self.invertFile = [];
    ...
    def readInvertedFile(self, inverted_file):
        ...
        print('done load inverted file took %g secs...' %(time.time()-tic));
        #print(time.time()-tic);
        return;
    ...
    def calScore(self, nGramList, k):
        ...
        for doc_id in range(self.docNum):
            denom_doc = 0.0;
            if doc_id not in self.doc_tf.keys():
                continue;
    ...
    def rocchio(self, docFilename, query_topic, query, output, k, iter, alpha, beta, gamma, relFeed):
